...
SEBT.views.goals = {
  ...
  renderList: () => {
    ...
    let filtered = goals.filter(g => {
      if(SEBT.views.goals.filterMode === 'in-progress' && g.status !== 'in-progress') return false;
      if(SEBT.views.goals.filterMode === 'completed' && g.status !== 'completed' && g.status !== 'purchased') return false;
      return true;
    });
    ...
    container.innerHTML = filtered.map(g => {
      ...
      <div class="card goal-card" style="padding: 24px; display:flex; flex-direction:column;">
        <div class="goal-header" style="margin-bottom: 16px; display:flex; justify-content:space-between; align-items:center;">
          ...
          <div style="text-align:right">
            <div style="font-size:0.75rem; color:var(--text-secondary); margin-bottom:2px;">Target</div>
            <div style="font-size:1.1rem; font-weight:700; color:var(--text-primary)">${formatCurrency(g.target)}</div>
          </div>
        </div>
        ...
        <div class="goal-meta" style="font-size: 0.8rem; display:flex; justify-content:space-between; color:var(--text-secondary); align-items:center; margin-bottom:auto;">
          ...
          <span class="badge" style="background:rgba(255,255,255,0.05); padding:2px 8px; border-radius:12px;">${g.status === 'in-progress' ? 'In Progress' : (g.status === 'purchased' ? 'Purchased' : 'Ready to Purchase')}</span>
        </div>
        <div style="margin-top: 16px; display:flex; gap: 8px; justify-content:flex-end;">
          ...
          ${g.status === 'completed' ? `<button class="btn btn-sm btn-primary" onclick="SEBT.views.goals.purchaseGoal(${g.id})">Purchase Item</button>` : ''}
          <button class="btn btn-sm btn-icon" style="color:var(--color-danger); border:1px solid transparent;" onclick="SEBT.views.goals.deleteGoal(${g.id})"><i class="bi bi-trash3"></i></button>
        </div>
      </div>`;
    }).join('');
  },
  ...
  saveFunds: (id) => {
    ...
    if(g) {
      ...
      SEBT.data.updateGoal(id, { saved: newSaved });
    ...
    if(g) {
      const confirmHtml = `
        ...
        <div class="modal-body" style="text-align: center; padding: 32px 24px;">
          ...
          <p style="color: var(--text-secondary); margin-bottom: 8px;">This will deduct <strong>₹${g.target}</strong> from your main balance.</p>
        </div>
  ...
  executePurchase: (id) => {
    ...
    if(g) {
      ...
      // Update goal status
      SEBT.data.updateGoal(id, { status: 'purchased' });
      SEBT.views.goals.renderList();
      
      // Instantly swap the modal content to Celebration Popup
      const congratsHtml = `
